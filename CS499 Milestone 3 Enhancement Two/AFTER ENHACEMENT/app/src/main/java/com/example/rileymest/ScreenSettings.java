...
public class ScreenSettings extends AppCompatActivity {
    ...
    private static final String KEY_GRID_SPAN = "grid_span"; // value for amount of grid columns
    ...
    protected void onCreate(Bundle savedInstanceState) { // ran when running java file
        ...
        int userId = new SessionManager(this).getCurrentUserId(); // gets current user logged in
        String username = (userId > 0) ? db.getUsernameById(userId) : null;
        textUsername.setText(username); // sets username of current user logged in
        ...
        savedSpan = Math.max(2, Math.min(5, savedSpan));
        seekBar.setMax(5);
    ...
    AlertDialog.Builder builder = new AlertDialog.Builder(this); // creates popup for user
